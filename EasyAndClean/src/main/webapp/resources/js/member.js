...
function idCheck() {
	...
	var url = "idCheck?memId="+document.frm.memId.value;
...
function bucketSuccess(){
	alert('장바구니에 담겼습니다.');	
	document.frm.action("reserve_insert");
	document.frm.submit();
...
function joinCheck() {
	...
	if (document.frm.memEmail.value.length == 0) {
		alert("이메일을 써주세요");
		frm.memId.focus();
		return false;
	}
...
function adminjoinCheck() {
	if (document.frm.memEmail.value.length == "") {
		alert("이메일을 써주세요");
		frm.memEmail.focus();
		return false;
	}
...
function bucketRes() {
	...
	var popupY= (window.screen.height / 2) - (popupHeight / 2);
	// 만들 팝업창 height 크기의 1/2 만큼 보정값으로 bucketSuccess빼주었음
