...
function attachAddr4(id){		
	...
	const str = `
		...
		<textarea cols="100" rows="5" name="NOTICE_COMMENT_CONTENT" style="border:none" placeholder="댓글을 입력하세요!"></textarea><input type="submit" id="listbutton" value="등록">
		</td>
		`;
...
function commCheck(){
	if(document.frm.NOTICE_COMMENT_CONTENT.value == ""){
		alert("댓글을 입력해주세요");
		frm.NOTICE_COMMENT_CONTENT.focus();
		return false;
	}
	if(document.frm.NOTICE_COMMENT_CONTENT.value == null){
		alert("댓글을 입력해주세요");
		frm.NOTICE_COMMENT_CONTENT.focus();
		return false;
	}
	if(document.frm.NOTICE_COMMENT_CONTENT.value.length == 0){
		alert("댓글을 입력해주세요");
		frm.NOTICE_COMMENT_CONTENT.focus();
		return false;
	}
