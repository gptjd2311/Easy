...
String pwd = (String) session.getAttribute("loginAdmin_pwd");
%>
</head>
<body class="no-sidebar">
		...
		<div class="container">
			<section>
				...
				<section>
					<header class="major">
						<h2>회원 예약 관리</h2>
						<span class="byline">배송완료된 예약 확인</span> <input type="hidden"
							name="memId" value=<%=id%>> <input type="hidden"
							name="memPwd" value=<%=pwd%>>
					</header>
					<form>
						<table class="mypageres">
							<p>
							<form action="reserveListSearchAll.do" method="post">
							...
							<c:forEach var="list" items="${reserve_listAll}">
								<tr class="mypage-bucket-table-line">
									...
									<td style="color: green;"><a
										href="ReserveServlet?command=reserve_admin_view_old&resId=${list.resId}">${list.resId}</td>
									<td>${list.proId}</td>
									<td>${list.proType}</td>
									<td>${list.proName}</td>
									<td>${list.resNum}</td>
	...
	<input type="button" id="btn_global_mypageres" value="예약 목록" onclick="location.href='reserve_listAll'">
					...
					<table id="mypageres_cen">
						<tr>
							<td colspan="15"><a
								href="reserveListSearchAll.do?page=${BSearchDto.firstPageNum}
		&pageDataCount=${BSearchDto.pageDataCount}
		&searchCol=${BSearchDto.searchCol}
		&searchVal=${BSearchDto.searchVal}">◀◀맨앞으로
							</a>&nbsp; <a
								href="reserveListSearchAll.do?page=${BSearchDto.prevPageNum}
		&pageDataCount=${BSearchDto.pageDataCount}
		&searchCol=${BSearchDto.searchCol}
		&searchVal=${BSearchDto.searchVal}">◀앞으로
									...
									<c:choose>
										<c:when test="${i eq BSearchDto.currentPageNum}">
											<a style="color: blue;"
												href="reserveListSearchAll.do?page=${i}
			&pageDataCount=${BSearchDto.pageDataCount}
			&searchCol=${BSearchDto.searchCol}
			...
			&searchVal=${BSearchDto.searchVal}">${i}
								...
								href="reserveListSearchAll.do?page=${BSearchDto.nextPageNum}
		&pageDataCount=${BSearchDto.pageDataCount}
		&searchCol=${BSearchDto.searchCol}
		&searchVal=${BSearchDto.searchVal}">뒤로▶
							</a>&nbsp; <a
								href="reserveListSearchAll.do?page=${BSearchDto.lastPageNum}
		&pageDataCount=${BSearchDto.pageDataCount}
		&searchCol=${BSearchDto.searchCol}
