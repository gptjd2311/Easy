		...
		<div class="container">
			<section>
				...
				<section>
					<header class="major">
						<h2>회원 예약 관리</h2>
						<span class="byline">예약 확인 및 배달 조회</span> <input type="hidden"
						...
						<table class="mypageres">
							<p>
								<span class="mypageresP">회원 아이디 입력&nbsp; <input
									...
									id="btn_global_mypageres2" value="조회" name="action"
							...
							<c:forEach var="list" items="${reserve_listAll}">
								<tr class="mypage-bucket-table-line">
									<td><input type="checkbox" name="chBox"
										value="${list.resId}">
									<td style="color: green;"><a href="reserve_admin_view?resId=${list.resId}"/>${list.resId}</td>
									<td>${list.resDate}</td>
									<td>${list.memId}</td>
									<td>${list.proId}</td>
									<td>${list.proType}</td>
